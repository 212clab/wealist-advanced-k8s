...
| 서비스 | 포트 | 비고 |
|--------|------|------|
| auth-service | 8080 | Spring Boot, OAuth2 |
| user-service | 8081 | Go |
| board-service | 8000 | Go |
| chat-service | 8001 | Go, WebSocket |
| noti-service | 8002 | Go, SSE |
| storage-service | 8003 | Go, S3/MinIO |
| video-service | 8004 | Go, LiveKit |
...
> auth-service는 DB를 사용하지 않음 (Redis만 사용)

---

## 서비스 간 통신 URL

### Docker-compose 환경 (localhost)
```
...
# Redis
REDIS_HOST=redis (K8s) / localhost (docker-compose)
REDIS_PORT=6379
...
3. **Secret 파일**: `.gitignore`에 `**/secret.yaml` 추가됨 (base는 예시용)
